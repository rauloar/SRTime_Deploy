    ...
    <div>
      {(modal === "add" || modal === "edit") && (
        <Modal title={modal === "add" ? "Crear Nuevo Turno" : "Editar Turno"} onClose={() => setModal(null)}>
          <Field label="Nombre del Turno:"><input value={current.name} onChange={e => setCurrent((p: any) => ({ ...p, name: e.target.value }))} /></Field>
          <Field label="Hora de Entrada:"><input type="time" value={current.expected_in} onChange={e => setCurrent((p: any) => ({ ...p, expected_in: e.target.value }))} /></Field>
          <Field label="Hora de Salida:"><input type="time" value={current.expected_out} onChange={e => setCurrent((p: any) => ({ ...p, expected_out: e.target.value }))} /></Field>
          <Field label="Minutos de Tolerancia:"><input type="number" min="0" value={current.grace_period_minutes} onChange={e => setCurrent((p: any) => ({ ...p, grace_period_minutes: e.target.value }))} /></Field>
